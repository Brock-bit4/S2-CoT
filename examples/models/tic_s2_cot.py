...
class SFA(nn.Module):
    """
    Spatial-Frequency Structural Fidelity Adapter: Feature Enhancement via Three Stages:
    1. Channel Excitation and Bottleneck Projection
    2. Spatial-Frequency Dual-Branch Modulation
    3. Soft Fusion
    """

    def __init__(self, in_dim: int = 128, middle_dim: int = 64, r: int = 16,
                 se_factor: float = 1.0,
        ...
        self.down1 = nn.Conv2d(in_dim, middle_dim, kernel_size=1)
        self.se_alpha = nn.Parameter(torch.tensor(se_factor))

        # Spatial branch
        self.s_gate = nn.Conv2d(in_dim, middle_dim, kernel_size=1)
        self.s_dw3 = nn.Conv2d(middle_dim, middle_dim, kernel_size=5, padding=2, groups=middle_dim)
        self.s_up_proj = nn.Conv2d(middle_dim, in_dim, kernel_size=1)

        # Frequency branch
        self.f_dw_conv = nn.Conv2d(middle_dim, middle_dim, kernel_size=3, padding=1, groups=middle_dim)
        self.amplitude_mlp = nn.Conv2d(middle_dim, middle_dim, kernel_size=1)
        self.f_up_proj = nn.Conv2d(middle_dim, in_dim, kernel_size=1)
        self.amplitude_gelu = nn.GELU()
        self.ifft_relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()

        # Shared
        self.sf_dw3 = nn.Conv2d(in_dim, in_dim, kernel_size=3, padding=1, groups=in_dim, bias=False)
        self.sf_relu = nn.ReLU()

        # Fusion
        self.sf_mlp = nn.Sequential(
            nn.Conv2d(in_dim * 2, in_dim // 2, kernel_size=1, bias=False),
            nn.ReLU(),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Channel
        c_weights = self.c_squeeze(x)
        c_weights = self.c_excite(c_weights)
        c = c_weights * x * self.se_factor
        down = self.down1(x + c)

        # Spatial branch
        gated = self.s_dw3(down) * self.s_gate(x)
        s = self.s_up_proj(torch.relu(gated))

        # Frequency branch
        fft = torch.fft.rfft2(down, dim=(2, 3), norm='backward')
        amplitude, phase = torch.abs(fft), torch.angle(fft)
        amplitude = self.amplitude_mlp(self.amplitude_gelu(self.f_dw_conv(amplitude)))
        modulated = amplitude * self.sigmoid(amplitude)
        ...
        f = self.f_up_proj(self.ifft_relu(ifft))

        # Shared
        s = self.sf_relu(self.sf_dw3(s))
        f = self.sf_relu(self.sf_dw3(f))
...
class SCA(nn.Module):
    ...
    def __init__(self, channels: int, reduction_ratio: int = 8, adapt_factor: float = 1.0):
        super().__init__()
        self.adapt_factor = adapt_factor
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        se = self.se_inter(refine)
        return se * refine * self.adapt_factor
