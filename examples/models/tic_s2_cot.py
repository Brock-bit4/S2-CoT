...
class SFA(nn.Module):
    """
    Spatial-Frequency Structural Fidelity Adapter (SFA): Feature Enhancement via Three Stages:
    1. Channel Excitation and Bottleneck Projection
    2. Spatial-Frequency Dual-Branch Modulation
    3. Soft Fusion
    """
    def __init__(self, in_dim=128, middle_dim=64, r=16, se_factor=1.0, adapt_factor=1.0):
        super().__init__()
        self.adapt_factor=adapt_factor

        # channel
        self.c_squeeze=nn.AdaptiveAvgPool2d(1)
        self.c_excite=nn.Sequential(
            nn.Conv2d(in_dim, in_dim//r, 1,bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_dim//r, in_dim, 1, bias=False),
            nn.Sigmoid()
        )
        self.down1=nn.Conv2d(in_dim, middle_dim, 1)
        # self.se_alpha = nn.Parameter(torch.tensor(se_factor))

        # spatial branch
        self.s_relu=nn.ReLU(inplace=True)
        self.s_gate=nn.Conv2d(in_dim, middle_dim, 1)
        self.s_dw3=nn.Conv2d(middle_dim, middle_dim, 5, padding=2, groups=middle_dim)
        self.s_up_proj=nn.Conv2d(middle_dim, in_dim, 1)

        # frequency branch
        self.f_dw_conv=nn.Conv2d(middle_dim, middle_dim, 3, padding=1, groups=middle_dim)
        self.amplitude_mlp=nn.Conv2d(middle_dim, middle_dim, 1)
        self.f_up_proj=nn.Conv2d(middle_dim, in_dim, 1)
        self.amplitude_gelu=nn.GELU()
        self.ifft_relu=nn.ReLU(inplace=True)
        self.sigmoid=nn.Sigmoid()

        # shared
        self.sf_dw3=nn.Conv2d(in_dim, in_dim, 3, padding=1, groups=in_dim, bias=False)
        self.sf_relu=nn.ReLU(inplace=True)

        # fusion
        self.sf_mlp=nn.Sequential(
            nn.Conv2d(in_dim*2, in_dim//2, 1, bias=False),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        # channel
        weight = self.c_squeeze(x)
        weight = self.c_excite(weight)
        c = x * weight  # c = weights * x * self.se_alpha
        base = self.down1(x + c)

        # spatial branch
        s_mod = self.s_up_proj(self.s_relu(self.s_dw3(base)*self.s_gate(x)))

        # frequency  branch
        fft = torch.fft.rfft2(base, dim=(2, 3), norm="backward")
        amplitude = torch.abs(fft)
        phase = torch.angle(fft)
        amp_mod = self.amplitude_mlp(self.amplitude_gelu(self.f_dw_conv(amplitude)))
        modulated = (amplitude*self.sigmoid(amp_mod))
        complex = torch.complex(modulated*torch.cos(phase), modulated * torch.sin(phase))
        ifft = torch.fft.irfft2(complex, s=(H, W), norm="backward")
        f_mod = self.f_up_proj(self.ifft_relu(ifft))

        # shared
        s = self.sf_relu(self.sf_dw3(s_mod))
        f = self.sf_relu(self.sf_dw3(f_mod))

        # fusion
        fused = self.sf_mlp(torch.cat([s, f], dim=1))

        return x + c + fused * self.adapt_factor


class SCA(nn.Module):
    """
    Semantic Context Adapter (SCA).
    """
    def __init__(self, channels, reduction_ratio=8, adapt_factor=1.0):
        super().__init__()
        self.adapt_factor = adapt_factor
        hidden_dim=max(4, channels//reduction_ratio)
        self.refine=nn.Sequential(
            nn.Conv2d(channels, hidden_dim, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(hidden_dim, channels, 1, bias=False)
        )
        self.se_inter=nn.Sequential(
    ...
    def forward(self,x):
        refine = x + self.refine(x)
        scale = self.se_inter (refine)
        return refine * scale # refine * scale * self.adapt_factor


class TIC_Modules(nn.Module):
    """
    ...
    def __init__(self, N=128, M=192, input_resolution=(256, 256), in_channel=3):
        ...
        self.encoder_S2_CoT = nn.Sequential(
            SFA(128, middle_dim=64), SFA(128, middle_dim=64), SFA(128, middle_dim=64)
        )
        self.decoder_S2_CoT = nn.Sequential(
            SFA(128, middle_dim=64), SFA(128, middle_dim=64), SFA(128, middle_dim=64)
        )

        self.ha_S2_CoT = SCA(N, reduction_ratio=8)
        self.hs_S2_CoT = SCA(N, reduction_ratio=8)
    ...
    def g_a(self, x, x_size=None):
        ...
        x = self.encoder_S2_CoT[0](x)
        x = self.g_a2(x)
        x = self.g_a3(x, (x_size[0] // 4, x_size[1] // 4))
        x = self.encoder_S2_CoT[1](x)
        x = self.g_a4(x)
        x = self.g_a5(x, (x_size[0] // 8, x_size[1] // 8))
        x = self.encoder_S2_CoT[2](x)
        x = self.g_a6(x)
    ...
    def g_s(self, x, x_size=None):
        ...
        x = self.g_s1(x)
        x = self.decoder_S2_CoT[2](x)
        x = self.g_s2(x, (x_size[0] // 8, x_size[1] // 8))
        x = self.g_s3(x)
        x = self.decoder_S2_CoT[1](x)
        x = self.g_s4(x, (x_size[0] // 4, x_size[1] // 4))
        x = self.g_s5(x)
        x = self.decoder_S2_CoT[0](x)
