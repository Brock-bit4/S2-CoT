...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        """
        # out_vis = dict()  # disvis
        B_, N, C = x.shape  # [B_, N, C]
        # qkv = self.qkv(x)  qkv shape: (B_, N, 3 * C)
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  # [3, B_, num_heads, N, head_dim]
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)  # [B_, num_heads, N, head_dim]
        ...
        # out_vis['inner_prod'] = attn.detach()  # disvis

        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww

        # out_vis['rpb'] = relative_position_bias.unsqueeze(0).detach()  # disvis
        ...
        # out_vis['attn'] = attn.detach()  # disvis

        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B_, N, C)   # x: (B_, N, C)
        x = self.proj(x)  # x: (B_, N, C)
        x = self.proj_drop(x)  # x: (B_, N, C)

        # return x, out_vis  # disvis
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        # self.actual_resolution = None  # disvis
        self.num_heads = num_heads
    ...
    def forward(self, x, x_size):
        # self.actual_resolution = x_size  # disvis
        H, W = x_size
        B, L, C = x.shape
        ...
        if self.input_resolution == x_size:
            # attn_windows, out_vis = self.attn(x_windows, mask=None)  # nW*B, window_size*window_size, C    # disvis
            attn_windows = self.attn(x_windows, mask=None)
        else:
            # attn_windows, out_vis = self.attn(x_windows, mask=None)  # disvis
            attn_windows = self.attn(x_windows, mask=None)
        ...
        if self.shift_size > 0:
            x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))   # [B, H, W, C]
        else:
            x = shifted_x
        x = x.view(B, H * W, C)
    ...
    def forward(self, x, x_size):
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                ...
                # x, _ = blk(x, x_size)  # disvis
                # attn = None  # disvis
                # attns.append(attn)  # disvis
                x = blk(x, x_size)

        # return x, attns  # disvis
        return x
...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # out, attns = self.residual_group(out, x_size)  # disvis
        # return self.patch_unembed(out, x_size) + x, attns  # disvis
