...
def test_epoch(test_dataloader, model, criterion_rd, criterion_task, predictor, evaluator):
    ...
    logging.info(f"[TEST RESULTS] | bpp loss: {bpp_loss.avg:.5f} | feat loss:{feat_loss.avg:.4f} | psnr: {psnr.avg:.5f}")
...
def parse_args(argv):
    ...
    # onlyTest: inference by trained model
    parser.add_argument("-ot", "--onlyTest", action='store_true', help='Just inference')
    parser.add_argument("-om", "--onlyTestModel", type=str, default=None, help='The model for inference')

    # test and val when TRAIN stage
...
def main(argv):
    ...
    # Only test mode: run inference with trained model
    if args.onlyTest:
        logging.info(f"Testing {args.onlyTestModel}. (only inference)")
        checkpoint = torch.load(args.onlyTestModel, map_location=device, weights_only=True)
